...
class RedisClient
{
    ...
    public function getSocket()
    {
        ...
        return isset($this->_pool[$connectionString]) ? $this->_pool[$connectionString] : null;
    }
    ...
    public function open()
    {
        ...
        $connectionString = $this->getConnectionString();
        $connectionTimeout = $this->connectionTimeout ?: ini_get('default_socket_timeout');
        $socket = @stream_socket_client($connectionString, $errorCode, $errorMessage, $connectionTimeout, $this->socketClientFlags);
        if (!$socket) {
            $message = "Failed to open redis DB connection ($connectionString): $errorCode - $errorMessage";
    ...
    private function parseResponse()
    {
        ...
        switch ($type) {
            case '+':
                return $this->parseOk($line);
            case '-':
                return $this->parseError($line);
            case ':':
                return $this->parseIneter($line);
            case '$':
                return $this->parseBulk($line);
            case '*':
                return $this->parseMultiBulk($line);
            default:
                throw new Exception('Received illegal data from redis: ' . $line);
        }
    }

    private function parseOk($line)
    {
    ...
    private function parseIneter($line)
    {
        return $line;
    }
    ...
    private function parseBulk($line)
    {
        if ($line == '-1') {
            return null;
        }
    ...
    private function parseMultiBulk($line)
    {
        $count = (int) $line;
        $data = [];
        for ($i = 0; $i < $count; $i++) {
